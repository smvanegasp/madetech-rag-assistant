"""QA generation and critique logic for multi-source eval questions."""
...
def build_document_groups(
    ...
    for i, anchor in enumerate(documents):
        ...
        neighbours = [
            documents[j] for j in top_idx if sims[j] >= min_sim
        ]
...
def generate_qa_pairs_from_multiple_documents(
    ...
    logger.info(
        "Docs %s: generated %d QA pairs", doc_titles, len(pairs)
    )
...
def generate_eval_dataset(
    doc_groups: List[List[HandbookDoc]],
    all_documents: List[HandbookDoc],
    n_groups: int = 10,
    seed: int = 42,
    ...
    Args:
        ...
        all_documents: Full list of handbook documents (used for the content lookup).
        n_groups: Number of document groups to sample.
        seed: Random seed for reproducible group sampling.
        questions_per_group: Number of QA pairs to generate per group.
        two_docs_ratio: Probability of using 2 docs (vs 3) per generation call.
    ...
    """
    random.seed(seed)
    sampled_groups = random.sample(doc_groups, min(n_groups, len(doc_groups)))
    logger.debug(
        "Sampled %d document groups (seed=%d) for eval generation",
        len(sampled_groups),
        seed,
    )
    ...
    for group in tqdm(sampled_groups, desc="Generating QA pairs", unit="group"):
        ...
        all_qa_pairs.extend(
            generate_qa_pairs_from_multiple_documents(
                group,
                num_questions=questions_per_group,
                model=model,
                two_docs_ratio=two_docs_ratio,
                max_tokens=max_tokens,
            )
        )
    ...
    for qa_pair in tqdm(all_qa_pairs, desc="Critiquing QA pairs", unit="pair"):
        ...
        # Multi-source: concatenate all source documents with a separator so
        # the critique LLM sees the full combined context.
        context = "\n\n---\n\n".join(
