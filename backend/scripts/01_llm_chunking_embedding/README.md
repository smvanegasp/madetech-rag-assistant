...
- Python 3.12+
- Backend dependencies (`pip install -e backend/` or `uv sync` in backend/)
...
```bash
python backend/scripts/01_llm_chunking_embedding/main.py
```
...
| `document_limit` | Cap documents for testing (`null` = process all) | `null` |
