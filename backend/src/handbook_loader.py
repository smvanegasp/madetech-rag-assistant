"""
@file handbook_loader.py
@description Loads handbook documents from markdown files with YAML frontmatter.

This module recursively loads all .md files from the data/handbook directory and
converts them into HandbookDoc objects. It handles:
- YAML frontmatter parsing (id, title, category)
- Fallback metadata extraction from file paths
- Content cleaning (strips frontmatter)
- Error handling for individual files

The loader is flexible: files can have full frontmatter, partial frontmatter,
or no frontmatter at all. Missing metadata is intelligently inferred from
...
def load_handbook_documents() -> List[HandbookDoc]:
    ...
    """
    handbook_dir = Path(__file__).parent.parent / "data" / "handbook"
    documents = []
